# typed: true

# DO NOT EDIT MANUALLY
# This is an autogenerated file for types exported from the `oauth` gem.
# Please instead update this file by running `bin/tapioca gem oauth`.


# pkg:gem/oauth#lib/oauth/client/net_http.rb:7
class Net::HTTPGenericRequest
  include ::OAuth::Helper

  # Add the OAuth information to an HTTP request. Depending on the <tt>options[:scheme]</tt> setting
  # this may add a header, additional query string parameters, or additional POST body parameters.
  # The default scheme is +header+, in which the OAuth parameters as put into the +Authorization+
  # header.
  #
  # * http - Configured Net::HTTP instance
  # * consumer - OAuth::Consumer instance
  # * token - OAuth::Token instance
  # * options - Request-specific options (e.g. +request_uri+, +consumer+, +token+, +scheme+,
  #   +signature_method+, +nonce+, +timestamp+, +body_hash+)
  #
  # This method also modifies the <tt>User-Agent</tt> header to add the OAuth gem version.
  #
  # See Also: {OAuth core spec version 1.0, section 5.4.1}[http://oauth.net/core/1.0#rfc.section.5.4.1],
  #           {OAuth Request Body Hash 1.0 Draft 4}[http://oauth.googlecode.com/svn/spec/ext/body_hash/1.0/drafts/4/spec.html,
  #                                                 http://oauth.googlecode.com/svn/spec/ext/body_hash/1.0/oauth-bodyhash.html#when_to_include]
  #
  # pkg:gem/oauth#lib/oauth/client/net_http.rb:28
  def oauth!(http, consumer = T.unsafe(nil), token = T.unsafe(nil), options = T.unsafe(nil)); end

  # pkg:gem/oauth#lib/oauth/client/net_http.rb:10
  def oauth_helper; end

  # Create a string suitable for signing for an HTTP request. This process involves parameter
  # normalization as specified in the OAuth specification. The exact normalization also depends
  # on the <tt>options[:scheme]</tt> being used so this must match what will be used for the request
  # itself. The default scheme is +header+, in which the OAuth parameters as put into the +Authorization+
  # header.
  #
  # * http - Configured Net::HTTP instance
  # * consumer - OAuth::Consumer instance
  # * token - OAuth::Token instance
  # * options - Request-specific options (e.g. +request_uri+, +consumer+, +token+, +scheme+,
  #   +signature_method+, +nonce+, +timestamp+)
  #
  # See Also: {OAuth core spec version 1.0, section 5.4.1}[http://oauth.net/core/1.0#rfc.section.5.4.1],
  #           {OAuth Request Body Hash 1.0 Draft 4}[http://oauth.googlecode.com/svn/spec/ext/body_hash/1.0/drafts/4/spec.html,
  #                                                 http://oauth.googlecode.com/svn/spec/ext/body_hash/1.0/oauth-bodyhash.html#when_to_include]
  #
  # pkg:gem/oauth#lib/oauth/client/net_http.rb:51
  def signature_base_string(http, consumer = T.unsafe(nil), token = T.unsafe(nil), options = T.unsafe(nil)); end

  private

  # pkg:gem/oauth#lib/oauth/client/net_http.rb:117
  def oauth_body_hash_required?(options); end

  # pkg:gem/oauth#lib/oauth/client/net_http.rb:73
  def oauth_full_request_uri(http, options); end

  # pkg:gem/oauth#lib/oauth/client/net_http.rb:60
  def oauth_helper_options(http, consumer, token, options); end

  # FIXME: if you're using a POST body and query string parameters, this method
  # will move query string parameters into the body unexpectedly. This may
  # cause problems with non-x-www-form-urlencoded bodies submitted to URLs
  # containing query string params. If duplicate parameters are present in both
  # places, all instances should be included when calculating the signature
  # base string.
  #
  # pkg:gem/oauth#lib/oauth/client/net_http.rb:132
  def set_oauth_body; end

  # pkg:gem/oauth#lib/oauth/client/net_http.rb:121
  def set_oauth_header; end

  # pkg:gem/oauth#lib/oauth/client/net_http.rb:140
  def set_oauth_query_string; end
end

# pkg:gem/oauth#lib/oauth/version.rb:3
module OAuth; end

# The Access Token is used for the actual "real" web service calls that you perform against the server
#
# pkg:gem/oauth#lib/oauth/tokens/access_token.rb:5
class OAuth::AccessToken < ::OAuth::ConsumerToken
  # Make a regular DELETE request using AccessToken
  #
  #   @response = @token.delete('/people/123')
  #   @response = @token.delete('/people/123', { 'Accept' => 'application/xml' })
  #
  # pkg:gem/oauth#lib/oauth/tokens/access_token.rb:81
  def delete(path, headers = T.unsafe(nil)); end

  # Make a regular GET request using AccessToken
  #
  #   @response = @token.get('/people')
  #   @response = @token.get('/people', { 'Accept'=>'application/xml' })
  #
  # pkg:gem/oauth#lib/oauth/tokens/access_token.rb:28
  def get(path, headers = T.unsafe(nil)); end

  # Make a regular HEAD request using AccessToken
  #
  #   @response = @token.head('/people')
  #
  # pkg:gem/oauth#lib/oauth/tokens/access_token.rb:36
  def head(path, headers = T.unsafe(nil)); end

  # Make a regular PATCH request using AccessToken
  #
  #   @response = @token.patch('/people/123')
  #   @response = @token.patch('/people/123', { :name => 'Bob', :email => 'bob@mailinator.com' })
  #   @response = @token.patch('/people/123', { :name => 'Bob', :email => 'bob@mailinator.com' }, { 'Accept' => 'application/xml' })
  #   @response = @token.patch('/people/123', nil, { 'Accept' => 'application/xml' })
  #   @response = @token.patch('/people/123', @person.to_xml, { 'Accept' => 'application/xml', 'Content-Type' => 'application/xml' })
  #
  # pkg:gem/oauth#lib/oauth/tokens/access_token.rb:72
  def patch(path, body = T.unsafe(nil), headers = T.unsafe(nil)); end

  # Make a regular POST request using AccessToken
  #
  #   @response = @token.post('/people')
  #   @response = @token.post('/people', { :name => 'Bob', :email => 'bob@mailinator.com' })
  #   @response = @token.post('/people', { :name => 'Bob', :email => 'bob@mailinator.com' }, { 'Accept' => 'application/xml' })
  #   @response = @token.post('/people', nil, {'Accept' => 'application/xml' })
  #   @response = @token.post('/people', @person.to_xml, { 'Accept'=>'application/xml', 'Content-Type' => 'application/xml' })
  #
  # pkg:gem/oauth#lib/oauth/tokens/access_token.rb:48
  def post(path, body = T.unsafe(nil), headers = T.unsafe(nil)); end

  # Make a regular PUT request using AccessToken
  #
  #   @response = @token.put('/people/123')
  #   @response = @token.put('/people/123', { :name => 'Bob', :email => 'bob@mailinator.com' })
  #   @response = @token.put('/people/123', { :name => 'Bob', :email => 'bob@mailinator.com' }, { 'Accept' => 'application/xml' })
  #   @response = @token.put('/people/123', nil, { 'Accept' => 'application/xml' })
  #   @response = @token.put('/people/123', @person.to_xml, { 'Accept' => 'application/xml', 'Content-Type' => 'application/xml' })
  #
  # pkg:gem/oauth#lib/oauth/tokens/access_token.rb:60
  def put(path, body = T.unsafe(nil), headers = T.unsafe(nil)); end

  # The less intrusive way. Otherwise, if we are to do it correctly inside consumer,
  # we need to restructure and touch more methods: request(), sign!(), etc.
  #
  # pkg:gem/oauth#lib/oauth/tokens/access_token.rb:8
  def request(http_method, path, *arguments); end
end

# pkg:gem/oauth#lib/oauth/client.rb:4
module OAuth::Client; end

# pkg:gem/oauth#lib/oauth/client/helper.rb:11
class OAuth::Client::Helper
  include ::OAuth::Helper

  # pkg:gem/oauth#lib/oauth/client/helper.rb:14
  def initialize(request, options = T.unsafe(nil)); end

  # pkg:gem/oauth#lib/oauth/client/helper.rb:77
  def amend_user_agent_header(headers); end

  # pkg:gem/oauth#lib/oauth/client/helper.rb:73
  def hash_body; end

  # pkg:gem/oauth#lib/oauth/client/helper.rb:87
  def header; end

  # pkg:gem/oauth#lib/oauth/client/helper.rb:22
  def nonce; end

  # pkg:gem/oauth#lib/oauth/client/helper.rb:30
  def oauth_parameters; end

  # pkg:gem/oauth#lib/oauth/client/helper.rb:20
  def options; end

  # pkg:gem/oauth#lib/oauth/client/helper.rb:97
  def parameters; end

  # pkg:gem/oauth#lib/oauth/client/helper.rb:101
  def parameters_with_oauth; end

  # pkg:gem/oauth#lib/oauth/client/helper.rb:51
  def signature(extra_options = T.unsafe(nil)); end

  # pkg:gem/oauth#lib/oauth/client/helper.rb:60
  def signature_base_string(extra_options = T.unsafe(nil)); end

  # pkg:gem/oauth#lib/oauth/client/helper.rb:26
  def timestamp; end

  # pkg:gem/oauth#lib/oauth/client/helper.rb:69
  def token_request?; end
end

# pkg:gem/oauth#lib/oauth/consumer.rb:11
class OAuth::Consumer
  # Create a new consumer instance by passing it a configuration hash:
  #
  #   @consumer = OAuth::Consumer.new(key, secret, {
  #     :site               => "http://term.ie",
  #     :scheme             => :header,
  #     :http_method        => :post,
  #     :request_token_path => "/oauth/example/request_token.php",
  #     :access_token_path  => "/oauth/example/access_token.php",
  #     :authorize_path     => "/oauth/example/authorize.php",
  #     :body_hash_enabled  => false
  #    })
  #
  # Start the process by requesting a token
  #
  #   @request_token = @consumer.get_request_token
  #   session[:request_token] = @request_token
  #   redirect_to @request_token.authorize_url
  #
  # When user returns create an access_token
  #
  #   @access_token = @request_token.get_access_token
  #   @photos=@access_token.get('/photos.xml')
  #
  # pkg:gem/oauth#lib/oauth/consumer.rb:106
  def initialize(consumer_key, consumer_secret, options = T.unsafe(nil)); end

  # pkg:gem/oauth#lib/oauth/consumer.rb:355
  def access_token_path; end

  # pkg:gem/oauth#lib/oauth/consumer.rb:384
  def access_token_url; end

  # pkg:gem/oauth#lib/oauth/consumer.rb:388
  def access_token_url?; end

  # pkg:gem/oauth#lib/oauth/consumer.rb:347
  def authenticate_path; end

  # pkg:gem/oauth#lib/oauth/consumer.rb:368
  def authenticate_url; end

  # pkg:gem/oauth#lib/oauth/consumer.rb:372
  def authenticate_url?; end

  # pkg:gem/oauth#lib/oauth/consumer.rb:351
  def authorize_path; end

  # pkg:gem/oauth#lib/oauth/consumer.rb:376
  def authorize_url; end

  # pkg:gem/oauth#lib/oauth/consumer.rb:380
  def authorize_url?; end

  # Creates and signs an http request.
  # It's recommended to use the Token classes to set this up correctly
  #
  # pkg:gem/oauth#lib/oauth/consumer.rb:274
  def create_signed_request(http_method, path, token = T.unsafe(nil), request_options = T.unsafe(nil), *arguments); end

  # pkg:gem/oauth#lib/oauth/consumer.rb:120
  def debug_output; end

  # Exchanges a verified Request Token for an Access Token.
  #
  # OAuth 1.0 vs 1.0a:
  # - 1.0a requires including oauth_verifier (as returned by the Provider after
  #   user authorization) when performing this exchange in a 3‑legged flow.
  # - 1.0 flows did not include oauth_verifier.
  #
  # Usage (3‑legged):
  #   access_token = request_token.get_access_token(oauth_verifier: params[:oauth_verifier])
  #
  # @param request_token [OAuth::RequestToken] The authorized request token
  # @param request_options [Hash] OAuth or request options (include :oauth_verifier for 1.0a)
  # @param arguments [Array] Optional POST body and headers
  # @yield [response_body] If a block is given, yields the raw response body.
  # @return [OAuth::AccessToken]
  #
  # pkg:gem/oauth#lib/oauth/consumer.rb:160
  def get_access_token(request_token, request_options = T.unsafe(nil), *arguments, &block); end

  # Makes a request to the service for a new OAuth::RequestToken
  #
  # Example:
  #   @request_token = @consumer.get_request_token
  #
  # To include OAuth parameters:
  #   @request_token = @consumer.get_request_token(
  #     oauth_callback: "http://example.com/cb"
  #   )
  #
  # To include application-specific parameters:
  #   @request_token = @consumer.get_request_token({}, foo: "bar")
  #
  # OAuth 1.0 vs 1.0a:
  # - In 1.0a, the Consumer SHOULD send oauth_callback when obtaining a request token
  #   (or explicitly use OUT_OF_BAND) and the Provider MUST include
  #   oauth_callback_confirmed=true in the response.
  # - This library defaults oauth_callback to OUT_OF_BAND ("oob") when not provided,
  #   which works for both 1.0 and 1.0a, and mirrors common provider behavior.
  # - The oauth_callback_confirmed response is parsed by the token classes; it is not
  #   part of the signature base string and thus is not signed.
  #
  # TODO: In a future major release, oauth_callback may be made mandatory unless
  #       request_options[:exclude_callback] is set, to reflect 1.0a guidance.
  #
  # @param request_options [Hash] OAuth options for the request. Notably
  #   :oauth_callback can be set to a URL, or OAuth::OUT_OF_BAND ("oob").
  # @param arguments [Array] Optional POST body and headers
  # @yield [response_body] If a block is given, yields the raw response body.
  # @return [OAuth::RequestToken]
  #
  # pkg:gem/oauth#lib/oauth/consumer.rb:202
  def get_request_token(request_options = T.unsafe(nil), *arguments, &block); end

  # The HTTP object for the site. The HTTP Object is what you get when you do Net::HTTP.new
  #
  # pkg:gem/oauth#lib/oauth/consumer.rb:131
  def http; end

  # The HTTP object for the site. The HTTP Object is what you get when you do Net::HTTP.new
  #
  # pkg:gem/oauth#lib/oauth/consumer.rb:81
  def http=(_arg0); end

  # The default http method
  #
  # pkg:gem/oauth#lib/oauth/consumer.rb:116
  def http_method; end

  # pkg:gem/oauth#lib/oauth/consumer.rb:80
  def key; end

  # pkg:gem/oauth#lib/oauth/consumer.rb:80
  def key=(_arg0); end

  # pkg:gem/oauth#lib/oauth/consumer.rb:80
  def options; end

  # pkg:gem/oauth#lib/oauth/consumer.rb:80
  def options=(_arg0); end

  # pkg:gem/oauth#lib/oauth/consumer.rb:392
  def proxy; end

  # Creates, signs and performs an http request.
  # It's recommended to use the OAuth::Token classes to set this up correctly.
  # request_options take precedence over consumer-wide options when signing
  #   a request.
  # arguments are POST and PUT bodies (a Hash, string-encoded parameters, or
  #   absent), followed by additional HTTP headers.
  #
  #   @consumer.request(:get,  '/people', @token, { :scheme => :query_string })
  #   @consumer.request(:post, '/people', @token, {}, @person.to_xml, { 'Content-Type' => 'application/xml' })
  #
  # pkg:gem/oauth#lib/oauth/consumer.rb:238
  def request(http_method, path, token = T.unsafe(nil), request_options = T.unsafe(nil), *arguments); end

  # pkg:gem/oauth#lib/oauth/consumer.rb:333
  def request_endpoint; end

  # pkg:gem/oauth#lib/oauth/consumer.rb:343
  def request_token_path; end

  # TODO: this is ugly, rewrite
  #
  # pkg:gem/oauth#lib/oauth/consumer.rb:360
  def request_token_url; end

  # pkg:gem/oauth#lib/oauth/consumer.rb:364
  def request_token_url?; end

  # pkg:gem/oauth#lib/oauth/consumer.rb:339
  def scheme; end

  # pkg:gem/oauth#lib/oauth/consumer.rb:80
  def secret; end

  # pkg:gem/oauth#lib/oauth/consumer.rb:80
  def secret=(_arg0); end

  # Sign the Request object. Use this if you have an externally generated http request object you want to sign.
  #
  # pkg:gem/oauth#lib/oauth/consumer.rb:320
  def sign!(request, token = T.unsafe(nil), request_options = T.unsafe(nil)); end

  # Return the signature_base_string
  #
  # pkg:gem/oauth#lib/oauth/consumer.rb:325
  def signature_base_string(request, token = T.unsafe(nil), request_options = T.unsafe(nil)); end

  # pkg:gem/oauth#lib/oauth/consumer.rb:329
  def site; end

  # pkg:gem/oauth#lib/oauth/consumer.rb:81
  def site=(_arg0); end

  # Creates a request and parses the result as url_encoded. This is used internally for the RequestToken and AccessToken requests.
  #
  # pkg:gem/oauth#lib/oauth/consumer.rb:281
  def token_request(http_method, path, token = T.unsafe(nil), request_options = T.unsafe(nil), *arguments); end

  # Contains the root URI for this site
  #
  # pkg:gem/oauth#lib/oauth/consumer.rb:136
  def uri(custom_uri = T.unsafe(nil)); end

  protected

  # Instantiates the http object
  #
  # pkg:gem/oauth#lib/oauth/consumer.rb:399
  def create_http(_url = T.unsafe(nil)); end

  # create the http request object for a given http_method and path
  #
  # pkg:gem/oauth#lib/oauth/consumer.rb:451
  def create_http_request(http_method, path, *arguments); end

  # pkg:gem/oauth#lib/oauth/consumer.rb:506
  def marshal_dump(*_args); end

  # pkg:gem/oauth#lib/oauth/consumer.rb:510
  def marshal_load(data); end
end

# pkg:gem/oauth#lib/oauth/consumer.rb:29
OAuth::Consumer::CA_FILE = T.let(T.unsafe(nil), String)

# pkg:gem/oauth#lib/oauth/consumer.rb:22
OAuth::Consumer::CA_FILES = T.let(T.unsafe(nil), Array)

# Superclass for tokens used by OAuth Clients
#
# pkg:gem/oauth#lib/oauth/tokens/consumer_token.rb:5
class OAuth::ConsumerToken < ::OAuth::Token
  # pkg:gem/oauth#lib/oauth/tokens/consumer_token.rb:15
  def initialize(consumer, token = T.unsafe(nil), secret = T.unsafe(nil)); end

  # pkg:gem/oauth#lib/oauth/tokens/consumer_token.rb:6
  def consumer; end

  # pkg:gem/oauth#lib/oauth/tokens/consumer_token.rb:6
  def consumer=(_arg0); end

  # pkg:gem/oauth#lib/oauth/tokens/consumer_token.rb:6
  def params; end

  # pkg:gem/oauth#lib/oauth/tokens/consumer_token.rb:6
  def params=(_arg0); end

  # Make a signed request using given http_method to the path
  #
  #   @token.request(:get,  '/people')
  #   @token.request(:post, '/people', @person.to_xml, { 'Content-Type' => 'application/xml' })
  #
  # pkg:gem/oauth#lib/oauth/tokens/consumer_token.rb:26
  def request(http_method, path, *arguments); end

  # pkg:gem/oauth#lib/oauth/tokens/consumer_token.rb:7
  def response; end

  # Sign a request generated elsewhere using Net:HTTP::Post.new or friends
  #
  # pkg:gem/oauth#lib/oauth/tokens/consumer_token.rb:31
  def sign!(request, options = T.unsafe(nil)); end

  class << self
    # pkg:gem/oauth#lib/oauth/tokens/consumer_token.rb:9
    def from_hash(consumer, hash); end
  end
end

# pkg:gem/oauth#lib/oauth/errors/error.rb:4
class OAuth::Error < ::StandardError; end

# pkg:gem/oauth#lib/oauth/helper.rb:8
module OAuth::Helper
  private

  # pkg:gem/oauth#lib/oauth/helper.rb:20
  def _escape(string); end

  # Escape +value+ by URL encoding all non-reserved character.
  #
  # See Also: {OAuth core spec version 1.0, section 5.1}[http://oauth.net/core/1.0#rfc.section.5.1]
  #
  # pkg:gem/oauth#lib/oauth/helper.rb:14
  def escape(value); end

  # Generate a random key of up to +size+ bytes. The value returned is Base64 encoded with non-word
  # characters removed.
  #
  # pkg:gem/oauth#lib/oauth/helper.rb:52
  def generate_key(size = T.unsafe(nil)); end

  # pkg:gem/oauth#lib/oauth/helper.rb:56
  def generate_nonce(size = T.unsafe(nil)); end

  # pkg:gem/oauth#lib/oauth/helper.rb:58
  def generate_timestamp; end

  # Normalize a +Hash+ of parameter values. Parameters are sorted by name, using lexicographical
  # byte value ordering. If two or more parameters share the same name, they are sorted by their value.
  # Parameters are concatenated in their sorted order into a single string. For each parameter, the name
  # is separated from the corresponding value by an "=" character, even if the value is empty. Each
  # name-value pair is separated by an "&" character.
  #
  # See Also: {OAuth core spec version 1.0, section 9.1.1}[http://oauth.net/core/1.0#rfc.section.9.1.1]
  #
  # pkg:gem/oauth#lib/oauth/helper.rb:69
  def normalize(params); end

  # Returns a string representation of the Hash like in URL query string
  # build_nested_query({:level_1 => {:level_2 => ['value_1','value_2']}}, 'prefix'))
  #   #=> ["prefix%5Blevel_1%5D%5Blevel_2%5D%5B%5D=value_1", "prefix%5Blevel_1%5D%5Blevel_2%5D%5B%5D=value_2"]
  #
  # pkg:gem/oauth#lib/oauth/helper.rb:94
  def normalize_nested_query(value, prefix = T.unsafe(nil)); end

  # Parse an Authorization / WWW-Authenticate header into a hash. Takes care of unescaping and
  # removing surrounding quotes. Raises a OAuth::Problem if the header is not parsable into a
  # valid hash. Does not validate the keys or values.
  #
  #   hash = parse_header(headers['Authorization'] || headers['WWW-Authenticate'])
  #   hash['oauth_timestamp']
  #     #=>"1234567890"
  #
  # pkg:gem/oauth#lib/oauth/helper.rb:117
  def parse_header(header); end

  # pkg:gem/oauth#lib/oauth/helper.rb:135
  def stringify_keys(hash); end

  # pkg:gem/oauth#lib/oauth/helper.rb:34
  def unescape(value); end

  class << self
    # pkg:gem/oauth#lib/oauth/helper.rb:20
    def _escape(string); end

    # Escape +value+ by URL encoding all non-reserved character.
    #
    # See Also: {OAuth core spec version 1.0, section 5.1}[http://oauth.net/core/1.0#rfc.section.5.1]
    #
    # pkg:gem/oauth#lib/oauth/helper.rb:14
    def escape(value); end

    # Generate a random key of up to +size+ bytes. The value returned is Base64 encoded with non-word
    # characters removed.
    #
    # pkg:gem/oauth#lib/oauth/helper.rb:52
    def generate_key(size = T.unsafe(nil)); end

    # pkg:gem/oauth#lib/oauth/helper.rb:58
    def generate_timestamp; end

    # Normalize a +Hash+ of parameter values. Parameters are sorted by name, using lexicographical
    # byte value ordering. If two or more parameters share the same name, they are sorted by their value.
    # Parameters are concatenated in their sorted order into a single string. For each parameter, the name
    # is separated from the corresponding value by an "=" character, even if the value is empty. Each
    # name-value pair is separated by an "&" character.
    #
    # See Also: {OAuth core spec version 1.0, section 9.1.1}[http://oauth.net/core/1.0#rfc.section.9.1.1]
    #
    # pkg:gem/oauth#lib/oauth/helper.rb:69
    def normalize(params); end

    # Returns a string representation of the Hash like in URL query string
    # build_nested_query({:level_1 => {:level_2 => ['value_1','value_2']}}, 'prefix'))
    #   #=> ["prefix%5Blevel_1%5D%5Blevel_2%5D%5B%5D=value_1", "prefix%5Blevel_1%5D%5Blevel_2%5D%5B%5D=value_2"]
    #
    # pkg:gem/oauth#lib/oauth/helper.rb:94
    def normalize_nested_query(value, prefix = T.unsafe(nil)); end

    # Parse an Authorization / WWW-Authenticate header into a hash. Takes care of unescaping and
    # removing surrounding quotes. Raises a OAuth::Problem if the header is not parsable into a
    # valid hash. Does not validate the keys or values.
    #
    #   hash = parse_header(headers['Authorization'] || headers['WWW-Authenticate'])
    #   hash['oauth_timestamp']
    #     #=>"1234567890"
    #
    # pkg:gem/oauth#lib/oauth/helper.rb:117
    def parse_header(header); end

    # pkg:gem/oauth#lib/oauth/helper.rb:135
    def stringify_keys(hash); end

    # pkg:gem/oauth#lib/oauth/helper.rb:34
    def unescape(value); end
  end
end

# Out-Of-Band callback token value.
# OAuth 1.0 and 1.0a both support out-of-band flows, where callbacks cannot be used.
# See RFC 5849 (OAuth 1.0), Section 6.1.1: Obtaining an Unauthorized Request Token
# and the 1.0a errata. Providers treating "oob" as the callback URL indicate that
# the verifier (for 1.0a) will be communicated out of band to the Consumer.
#
# pkg:gem/oauth#lib/oauth/oauth.rb:9
OAuth::OUT_OF_BAND = T.let(T.unsafe(nil), String)

# OAuth parameter keys this library recognizes when normalizing/signing requests.
# Notes on 1.0 vs 1.0a:
# - oauth_verifier: Introduced by OAuth 1.0a. Returned to the Consumer after user
#   authorization and required when exchanging a Request Token for an Access Token
#   (Section 6.3.1 in RFC 5849 / 1.0a change).
# - oauth_callback: Present in 1.0; 1.0a clarified that the Consumer MUST send it when
#   obtaining a Request Token (or use "oob") and that the Service Provider MUST return
#   oauth_callback_confirmed=true with the Request Token response to prevent session
#   fixation attacks. Note that oauth_callback_confirmed is a response parameter, not
#   a request signing parameter, and thus is not listed here.
# Other keys are common to both 1.0 and 1.0a.
#
# pkg:gem/oauth#lib/oauth/oauth.rb:22
OAuth::PARAMETERS = T.let(T.unsafe(nil), Array)

# pkg:gem/oauth#lib/oauth/errors/problem.rb:4
class OAuth::Problem < ::OAuth::Unauthorized
  # pkg:gem/oauth#lib/oauth/errors/problem.rb:7
  def initialize(problem, request = T.unsafe(nil), params = T.unsafe(nil)); end

  # pkg:gem/oauth#lib/oauth/errors/problem.rb:5
  def params; end

  # pkg:gem/oauth#lib/oauth/errors/problem.rb:5
  def problem; end

  # pkg:gem/oauth#lib/oauth/errors/problem.rb:13
  def to_s; end
end

# reserved character regexp, per section 5.1
#
# pkg:gem/oauth#lib/oauth/oauth.rb:36
OAuth::RESERVED_CHARACTERS = T.let(T.unsafe(nil), Regexp)

# pkg:gem/oauth#lib/oauth/request_proxy.rb:4
module OAuth::RequestProxy
  class << self
    # pkg:gem/oauth#lib/oauth/request_proxy.rb:5
    def available_proxies; end

    # pkg:gem/oauth#lib/oauth/request_proxy.rb:9
    def proxy(request, options = T.unsafe(nil)); end
  end
end

# pkg:gem/oauth#lib/oauth/request_proxy/base.rb:8
class OAuth::RequestProxy::Base
  include ::OAuth::Helper

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:17
  def initialize(request, options = T.unsafe(nil)); end

  # TODO: deprecate these
  #
  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:71
  def consumer_key; end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:92
  def non_oauth_parameters; end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:73
  def nonce; end

  # See 9.1.1. in specs Normalize Request Parameters
  #
  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:107
  def normalized_parameters; end

  # See 9.1.2 in specs
  #
  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:101
  def normalized_uri; end

  # OAuth parameters
  #
  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:25
  def oauth_callback; end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:29
  def oauth_consumer_key; end

  # Authorization header for OAuth
  #
  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:148
  def oauth_header(options = T.unsafe(nil)); end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:33
  def oauth_nonce; end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:88
  def oauth_parameters; end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:37
  def oauth_signature; end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:42
  def oauth_signature_method; end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:51
  def oauth_timestamp; end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:55
  def oauth_token; end

  # OAuth 1.0a only: value returned to the Consumer after user authorization
  # and required when exchanging a Request Token for an Access Token.
  # Not present in OAuth 1.0 flows.
  #
  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:62
  def oauth_verifier; end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:66
  def oauth_version; end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:15
  def options; end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:15
  def options=(_arg0); end

  # Parameter accessors
  #
  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:80
  def parameters; end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:84
  def parameters_for_signature; end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:155
  def query_string_blank?; end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:15
  def request; end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:15
  def request=(_arg0); end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:111
  def sign(options = T.unsafe(nil)); end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:115
  def sign!(options = T.unsafe(nil)); end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:75
  def signature; end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:96
  def signature_and_unsigned_parameters; end

  # See 9.1 in specs
  #
  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:122
  def signature_base_string; end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:76
  def signature_method; end

  # Has this request been signed yet?
  #
  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:128
  def signed?; end

  # URI, including OAuth parameters
  #
  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:133
  def signed_uri(with_oauth: T.unsafe(nil)); end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:74
  def timestamp; end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:72
  def token; end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:15
  def unsigned_parameters; end

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:15
  def unsigned_parameters=(_arg0); end

  protected

  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:165
  def header_params; end

  # Utility to make parameter values array-style (or keep nil) so that
  # subclasses can rely on array values for parameter merging/signing.
  # Mirrors the implementation previously present in
  # ActionDispatchRequest#wrap_values.
  #
  # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:188
  def wrap_values(hash); end

  class << self
    # pkg:gem/oauth#lib/oauth/request_proxy/base.rb:11
    def proxies(klass); end
  end
end

# RequestProxy for Hashes to facilitate simpler signature creation.
# Usage:
#   request = OAuth::RequestProxy.proxy \
#      "method" => "iq",
#      "uri"    => [from, to] * "&",
#      "parameters" => {
#        "oauth_consumer_key"     => oauth_consumer_key,
#        "oauth_token"            => oauth_token,
#        "oauth_signature_method" => "HMAC-SHA1"
#      }
#
#   signature = OAuth::Signature.sign \
#     request,
#     :consumer_secret => oauth_consumer_secret,
#     :token_secret    => oauth_token_secret,
#
# pkg:gem/oauth#lib/oauth/request_proxy/mock_request.rb:22
class OAuth::RequestProxy::MockRequest < ::OAuth::RequestProxy::Base
  # pkg:gem/oauth#lib/oauth/request_proxy/mock_request.rb:29
  def method; end

  # pkg:gem/oauth#lib/oauth/request_proxy/mock_request.rb:33
  def normalized_uri; end

  # pkg:gem/oauth#lib/oauth/request_proxy/mock_request.rb:25
  def parameters; end

  # pkg:gem/oauth#lib/oauth/request_proxy/mock_request.rb:41
  def uri; end
end

# pkg:gem/oauth#lib/oauth/request_proxy/net_http.rb:10
module OAuth::RequestProxy::Net; end

# pkg:gem/oauth#lib/oauth/request_proxy/net_http.rb:11
module OAuth::RequestProxy::Net::HTTP; end

# pkg:gem/oauth#lib/oauth/request_proxy/net_http.rb:12
class OAuth::RequestProxy::Net::HTTP::HTTPRequest < ::OAuth::RequestProxy::Base
  # pkg:gem/oauth#lib/oauth/request_proxy/net_http.rb:31
  def body; end

  # pkg:gem/oauth#lib/oauth/request_proxy/net_http.rb:15
  def method; end

  # pkg:gem/oauth#lib/oauth/request_proxy/net_http.rb:23
  def parameters; end

  # pkg:gem/oauth#lib/oauth/request_proxy/net_http.rb:19
  def uri; end

  private

  # pkg:gem/oauth#lib/oauth/request_proxy/net_http.rb:37
  def all_parameters; end

  # pkg:gem/oauth#lib/oauth/request_proxy/net_http.rb:71
  def auth_header_params; end

  # pkg:gem/oauth#lib/oauth/request_proxy/net_http.rb:59
  def form_url_encoded?; end

  # pkg:gem/oauth#lib/oauth/request_proxy/net_http.rb:67
  def post_params; end

  # pkg:gem/oauth#lib/oauth/request_proxy/net_http.rb:63
  def query_params; end

  # pkg:gem/oauth#lib/oauth/request_proxy/net_http.rb:51
  def query_string; end
end

# pkg:gem/oauth#lib/oauth/request_proxy.rb:25
class OAuth::RequestProxy::UnknownRequestType < ::RuntimeError; end

# The RequestToken is used for the initial Request.
# This is normally created by the Consumer object.
#
# pkg:gem/oauth#lib/oauth/tokens/request_token.rb:6
class OAuth::RequestToken < ::OAuth::ConsumerToken
  # pkg:gem/oauth#lib/oauth/tokens/request_token.rb:15
  def authenticate_url(params = T.unsafe(nil)); end

  # Generate an authorization URL for user authorization
  #
  # pkg:gem/oauth#lib/oauth/tokens/request_token.rb:8
  def authorize_url(params = T.unsafe(nil)); end

  # pkg:gem/oauth#lib/oauth/tokens/request_token.rb:22
  def callback_confirmed?; end

  # exchange for AccessToken on server
  #
  # pkg:gem/oauth#lib/oauth/tokens/request_token.rb:27
  def get_access_token(options = T.unsafe(nil), *arguments); end

  protected

  # construct an authorization or authentication url
  #
  # pkg:gem/oauth#lib/oauth/tokens/request_token.rb:41
  def build_url(base_url, params); end
end

# Used on the server for generating tokens
#
# pkg:gem/oauth#lib/oauth/tokens/server_token.rb:5
class OAuth::ServerToken < ::OAuth::Token
  # pkg:gem/oauth#lib/oauth/tokens/server_token.rb:6
  def initialize; end
end

# pkg:gem/oauth#lib/oauth/signature.rb:4
module OAuth::Signature
  class << self
    # Returns a list of available signature methods
    #
    # pkg:gem/oauth#lib/oauth/signature.rb:6
    def available_methods; end

    # Create the body hash for a request
    #
    # pkg:gem/oauth#lib/oauth/signature.rb:42
    def body_hash(request, options = T.unsafe(nil), &block); end

    # Build a signature from a +request+.
    #
    # Raises UnknownSignatureMethod exception if the signature method is unknown.
    #
    # pkg:gem/oauth#lib/oauth/signature.rb:13
    def build(request, options = T.unsafe(nil), &block); end

    # Sign a +request+
    #
    # pkg:gem/oauth#lib/oauth/signature.rb:25
    def sign(request, options = T.unsafe(nil), &block); end

    # Create the signature base string for +request+. This string is the normalized parameter information.
    #
    # See Also: {OAuth core spec version 1.0, section 9.1.1}[http://oauth.net/core/1.0#rfc.section.9.1.1]
    #
    # pkg:gem/oauth#lib/oauth/signature.rb:37
    def signature_base_string(request, options = T.unsafe(nil), &block); end

    # Verify the signature of +request+
    #
    # pkg:gem/oauth#lib/oauth/signature.rb:30
    def verify(request, options = T.unsafe(nil), &block); end
  end
end

# pkg:gem/oauth#lib/oauth/signature/base.rb:10
class OAuth::Signature::Base
  include ::OAuth::Helper

  # pkg:gem/oauth#lib/oauth/signature/base.rb:23
  def initialize(request, options = T.unsafe(nil), &block); end

  # pkg:gem/oauth#lib/oauth/signature/base.rb:58
  def ==(other); end

  # pkg:gem/oauth#lib/oauth/signature/base.rb:72
  def body_hash; end

  # pkg:gem/oauth#lib/oauth/signature/base.rb:14
  def consumer_secret; end

  # pkg:gem/oauth#lib/oauth/signature/base.rb:13
  def options; end

  # pkg:gem/oauth#lib/oauth/signature/base.rb:13
  def options=(_arg0); end

  # pkg:gem/oauth#lib/oauth/signature/base.rb:14
  def request; end

  # pkg:gem/oauth#lib/oauth/signature/base.rb:54
  def signature; end

  # pkg:gem/oauth#lib/oauth/signature/base.rb:68
  def signature_base_string; end

  # pkg:gem/oauth#lib/oauth/signature/base.rb:14
  def token_secret; end

  # pkg:gem/oauth#lib/oauth/signature/base.rb:64
  def verify; end

  private

  # pkg:gem/oauth#lib/oauth/signature/base.rb:82
  def consumer_key; end

  # pkg:gem/oauth#lib/oauth/signature/base.rb:94
  def digest; end

  # pkg:gem/oauth#lib/oauth/signature/base.rb:86
  def nonce; end

  # pkg:gem/oauth#lib/oauth/signature/base.rb:98
  def raise_instantiation_error; end

  # pkg:gem/oauth#lib/oauth/signature/base.rb:90
  def secret; end

  # pkg:gem/oauth#lib/oauth/signature/base.rb:78
  def token; end

  class << self
    # pkg:gem/oauth#lib/oauth/signature/base.rb:16
    def implements(signature_method = T.unsafe(nil)); end
  end
end

# pkg:gem/oauth#lib/oauth/signature/hmac/sha1.rb:7
module OAuth::Signature::HMAC; end

# pkg:gem/oauth#lib/oauth/signature/hmac/sha1.rb:8
class OAuth::Signature::HMAC::SHA1 < ::OAuth::Signature::Base
  # pkg:gem/oauth#lib/oauth/signature/hmac/sha1.rb:11
  def body_hash; end

  private

  # pkg:gem/oauth#lib/oauth/signature/hmac/sha1.rb:17
  def digest; end
end

# pkg:gem/oauth#lib/oauth/signature/hmac/sha256.rb:8
class OAuth::Signature::HMAC::SHA256 < ::OAuth::Signature::Base
  # pkg:gem/oauth#lib/oauth/signature/hmac/sha256.rb:11
  def body_hash; end

  private

  # pkg:gem/oauth#lib/oauth/signature/hmac/sha256.rb:17
  def digest; end
end

# pkg:gem/oauth#lib/oauth/signature/plaintext.rb:7
class OAuth::Signature::PLAINTEXT < ::OAuth::Signature::Base
  # pkg:gem/oauth#lib/oauth/signature/plaintext.rb:14
  def ==(other); end

  # pkg:gem/oauth#lib/oauth/signature/plaintext.rb:22
  def body_hash; end

  # pkg:gem/oauth#lib/oauth/signature/plaintext.rb:10
  def signature; end

  # pkg:gem/oauth#lib/oauth/signature/plaintext.rb:18
  def signature_base_string; end
end

# pkg:gem/oauth#lib/oauth/signature/rsa/sha1.rb:7
module OAuth::Signature::RSA; end

# pkg:gem/oauth#lib/oauth/signature/rsa/sha1.rb:8
class OAuth::Signature::RSA::SHA1 < ::OAuth::Signature::Base
  # pkg:gem/oauth#lib/oauth/signature/rsa/sha1.rb:11
  def ==(other); end

  # pkg:gem/oauth#lib/oauth/signature/rsa/sha1.rb:27
  def body_hash; end

  # pkg:gem/oauth#lib/oauth/signature/rsa/sha1.rb:16
  def public_key; end

  private

  # pkg:gem/oauth#lib/oauth/signature/rsa/sha1.rb:40
  def decode_public_key; end

  # pkg:gem/oauth#lib/oauth/signature/rsa/sha1.rb:49
  def digest; end
end

# pkg:gem/oauth#lib/oauth/signature.rb:46
class OAuth::Signature::UnknownSignatureMethod < ::RuntimeError; end

# Superclass for the various tokens used by OAuth
#
# pkg:gem/oauth#lib/oauth/tokens/token.rb:5
class OAuth::Token
  include ::OAuth::Helper

  # pkg:gem/oauth#lib/oauth/tokens/token.rb:10
  def initialize(token, secret); end

  # pkg:gem/oauth#lib/oauth/tokens/token.rb:8
  def secret; end

  # pkg:gem/oauth#lib/oauth/tokens/token.rb:8
  def secret=(_arg0); end

  # pkg:gem/oauth#lib/oauth/tokens/token.rb:15
  def to_query; end

  # pkg:gem/oauth#lib/oauth/tokens/token.rb:8
  def token; end

  # pkg:gem/oauth#lib/oauth/tokens/token.rb:8
  def token=(_arg0); end
end

# pkg:gem/oauth#lib/oauth/errors/unauthorized.rb:4
class OAuth::Unauthorized < ::OAuth::Error
  # pkg:gem/oauth#lib/oauth/errors/unauthorized.rb:7
  def initialize(request = T.unsafe(nil)); end

  # pkg:gem/oauth#lib/oauth/errors/unauthorized.rb:5
  def request; end

  # pkg:gem/oauth#lib/oauth/errors/unauthorized.rb:12
  def to_s; end
end

# pkg:gem/oauth#lib/oauth/version.rb:4
module OAuth::Version
  extend ::VersionGem::Basic
  extend ::VersionGem::Api
end

# pkg:gem/oauth#lib/oauth/version.rb:5
OAuth::Version::VERSION = T.let(T.unsafe(nil), String)
