...
Pantria's [calendar](calendar.md) can two-way-sync with **Google Calendar**:
events you create in Pantria appear in Google, and changes in Google flow back
into Pantria. There is **one connection per instance**, configured through the
UI by an admin — Pantria serves a single household, so a single shared calendar
matches the deployment model.

!!! note "You provide your own Google OAuth client"
    Pantria is self-hosted, so there is no shared Pantria Google app. You create
...
3. **Select a calendar** — choose which of your Google calendars Pantria should
   sync with.
...
- **Pantria → Google.** Creating, updating or deleting a local event enqueues a
  `CalendarPushJob` that writes the change to Google.
- **Google → Pantria.** A recurring job polls Google every 5 minutes using an
...
- **Loop prevention.** Each event tracks its `sync_origin`, `remote_id` and
  ...
  change Pantria pushed is not re-imported — mirroring the same echo-guard
...
- Model: [`app/models/calendar_connection.rb`](https://github.com/SGraef/Pantria/blob/main/app/models/calendar_connection.rb)
- Controller: [`app/controllers/calendar_connections_controller.rb`](https://github.com/SGraef/Pantria/blob/main/app/controllers/calendar_connections_controller.rb)
- Sync services: [`app/services/calendar_sync/`](https://github.com/SGraef/Pantria/tree/main/app/services/calendar_sync)
- Jobs: [`app/jobs/calendar_poll_job.rb`](https://github.com/SGraef/Pantria/blob/main/app/jobs/calendar_poll_job.rb),
  [`app/jobs/calendar_push_job.rb`](https://github.com/SGraef/Pantria/blob/main/app/jobs/calendar_push_job.rb)
