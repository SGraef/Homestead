...
Homestead is self-hosted, so you provide your own Google OAuth credentials (there
is no shared Homestead Google app). This is a one-time setup, ~10–15 minutes.
...
- Find your Homestead **redirect URI**. Open **Calendar → Calendar sync** in
  Homestead — it's shown on that page. It's your instance URL + `/calendar_connection/callback`, e.g.
...
4. **Scopes** step → **Add or remove scopes** → add:
   ...
   ```
   (Full read/write calendar access — Homestead needs to create, update and delete
...
5. **Publishing status** — this matters for how long a connection lasts:
   - **Recommended: "Publish app" → In production.** Because `…/auth/calendar`
     ...
     **Advanced → Go to Homestead (unsafe)** to proceed. This is fine for your own
...
5. **Authorized redirect URIs → + Add URI** → paste your redirect URI from
   ...
   - "Authorized JavaScript origins" can be left empty (Homestead uses the
     server-side flow).
...
3. Click **Connect Google Calendar** → Google consent (click through the
   ...
   Homestead, now **Connected**.
4. **Choose which calendar** to sync from the picker → **Save**.
5. Done. Homestead pulls changes every ~5 minutes (or click **Sync now**) and
...
| Symptom | Cause / fix |
|---|---|
| **`redirect_uri_mismatch`** at Google | The redirect URI in the client must match Homestead's **exactly** (scheme, host, port, path, no trailing slash). Copy it from the settings page. |
...
> **Note on credentials & rotation:** Homestead encrypts the client secret and the
